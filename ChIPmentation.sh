...
#          BUGS: ---
#         NOTES: ---
#       AUTHORS: Derek Ng, Darrell Ricke - Darrell.Ricke@ll.mit.edu
...
s_seq_lengths() {
    ...
    if [[ $types == SE ]]
    then
        /D/fqSeqLen -n $nameLen -s $seqLen -o $iof/$name'_seqLen.txt' /io/$file1 \
    ...
    then
        /D/fqSeqLen -n $nameLen -s $seqLen -o $iof/$name'_seqLen.txt' /io/$file1 /io/$file2 \
            && echo -e "${green}Step: Count reads and calculate read lengths DONE$nc" 2>&1 | tee -a $iof/QC_pipe_processing.log \
            || { echo -e "${red}Step: Count reads and calculate read lengths FAIL$nc" 2>&1 | tee -a $iof/QC_pipe_processing.log && ok=false; }
    fi
...
s_genrich_peaks() {
    echo -e "\n\n**************************************************"
    echo -e "${cyan}Step 10: Genrich$nc"
    if [[ $types == SE ]]
    then
...
s_postprocess() {
    ...
    python /D/QcParser.py $name chipmentation
    echo "QcParser done"
